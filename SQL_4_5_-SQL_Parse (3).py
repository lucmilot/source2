...
def sql_parse():
    ...
    out_file_investigate = out_dir + "/SQL_VIEW_INVESTIGATE_" + in_file.split(r"_")[-1:][0]
    out_file_crud = out_dir + "/SQL_VIEW_CRUD_" + in_file.split(r"_")[-1:][0]    
    ...
    if append_sw == 0:
        ...
        df_investigate.to_csv(out_file_investigate,mode = 'w',header=True, index = False)  
        df_crud.to_csv(out_file_crud,mode = 'w',header=True, index = False)
    else :
        df_select.to_csv(out_file_select,mode = 'a',header=False, index = False)
        df_update.to_csv(out_file_update,mode = 'a',header=False, index = False)
        df_insert.to_csv(out_file_insert,mode = 'a',header=False, index = False)
        df_delete.to_csv(out_file_delete,mode = 'a',header=False, index = False)
        df_investigate.to_csv(out_file_investigate,mode = 'a',header=False, index = False)  
