...
def sql_parse(df_out_mbr):
    ...
    if not os.path.isfile(out_file_SQL):
       df_out_mbr.to_csv(out_file_SQL,mode = 'w',header=True, index = False)
    else: # else it exists so append without writing the header
       df_out_mbr.to_csv(out_file_SQL,mode = 'a',header=False, index = False) 
...
def main_process():    
    ...
    if os.path.exists(out_file): os.remove(out_file)	 
    if os.path.exists(out_file_SQL): os.remove(out_file_SQL)
    if os.path.exists(out_file_s): os.remove(out_file_s)	
    if os.path.exists(out_file_u): os.remove(out_file_u)	
    if os.path.exists(out_file_i): os.remove(out_file_i)	
    if os.path.exists(out_file_d): os.remove(out_file_d)	      
...
def main_in(argv):
    global in_file, out_file, out_file_s, out_file_i, out_file_u, out_file_d, out_file_x, out_file_SQL, username, password    
    ...
    out_file_SQL = wrk_dir + "\\" + out_file_1 + "_SQL.CSV"  
